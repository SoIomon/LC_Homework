...
int findVictim(BufPool pool)
{
	...
	for (int i = pool->clock; i < MAX_USAGE * pool->nbufs; i++) {
		if (pool->bufs[i % pool->nbufs].pin == 0 && pool->bufs[i % pool->nbufs].pop == 0) {
			
			if (pool->bufs[i % pool->nbufs].dirty != 0) {
				pool->nwrites += 1;
			}

			pool->clock = (i + 1) % pool->nbufs;
			return i % pool->nbufs;
		}
		else {
			if (pool->bufs[i % pool->nbufs].pop > 0) {
				pool->bufs[i % pool->nbufs].pop -= 1;
			}
		}
